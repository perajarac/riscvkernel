...
void * MemoryAllocator::kernel_mem_alloc(size_t sz)
{
    ...
    for(FreeMemory* temp = head; temp; temp = temp->next){
        if(temp->size>=sz+1){
            ...
            if(temp->prev)temp->prev->next = leftOver;
            else head = leftOver;
            if(temp->next)temp->next->prev = leftOver;
...
{
    if((size_t*)adr > (size_t*)HEAP_END_ADDR || (size_t*)adr < (size_t*)HEAP_START_ADDR) return -1;

    adr = reinterpret_cast<void*>(reinterpret_cast<size_t>(adr) - MEM_BLOCK_SIZE);

    FreeMemory* prev = 0;
    if(head == nullptr || (size_t*)adr < (size_t*)head) prev = 0;
    else{
